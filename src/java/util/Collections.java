...
public class Collections {
	...
	/*
	 * Tuning parameters for algorithms - Many of the List algorithms have two implementations, one of
	 * which is appropriate for RandomAccess lists, the other for "sequential." Often, the random access
	 * variant yields better performance on small sequential access lists. The tuning parameters below
	 * determine the cutoff point for what constitutes a "small" sequential access list for each
	 * algorithm. The values below were empirically determined to work well for LinkedList. Hopefully
	 * they should be reasonable for other sequential access List implementations. Those doing
	 * performance work on this code would do well to validate the values of these parameters from time
	...
	/**
	 * Sorts the specified list into ascending order, according to the {@linkplain Comparable natural
	 * ordering} of its elements. All elements in the list must implement the {@link Comparable}
	 * interface. Furthermore, all elements in the list must be <i>mutually comparable</i> (that is,
	 * {@code e1.compareTo(e2)} must not throw a {@code ClassCastException} for any elements {@code e1}
	 ...
	 * array. It is well-suited to merging two or more sorted arrays: simply concatenate the arrays and
	 * sort the resulting array.
	 *
	 * <p>
	 * The implementation was adapted from Tim Peters's list sort for Python
	 * (<a href="http://svn.python.org/projects/python/trunk/Objects/listsort.txt"> TimSort</a>). It
	 * uses techiques from Peter McIlroy's "Optimistic Sorting and Information Theoretic Complexity", in
	 ...
	 * n<sup>2</sup> log(n) performance that would result from attempting to sort a linked list in
	 * place.
	 *
	 ...
	 * @throws UnsupportedOperationException if the specified list's list-iterator does not support the
	 *                                       {@code set} operation.
	...
	/**
	 * Sorts the specified list according to the order induced by the specified comparator. All elements
	 * in the list must be <i>mutually comparable</i> using the specified comparator (that is,
	 * {@code c.compare(e1, e2)} must not throw a {@code ClassCastException} for any elements {@code e1}
	 * and {@code e2} in the list).
	 *
	 * <p>
	 * This sort is guaranteed to be <i>stable</i>: equal elements will not be reordered as a result of
	...
	/**
	 * Searches the specified list for the specified object using the binary search algorithm. The list
	 * must be sorted into ascending order according to the {@linkplain Comparable natural ordering} of
	 * its elements (as by the {@link #sort(List)} method) prior to making this call. If it is not
	 * sorted, the results are undefined. If the list contains multiple elements equal to the specified
	 ...
	 * and is large, this method will do an iterator-based binary search that performs O(n) link
	 ...
	 *         greater than the key, or <tt>list.size()</tt> if all elements in the list are less than
	 *         the specified key. Note that this guarantees that the return value will be &gt;= 0 if and
	 *         only if the key is found.
	 * @throws ClassCastException if the list contains elements that are not <i>mutually comparable</i>
	 *                            (for example, strings and integers), or the search key is not mutually
	 ...
	 * {@link #sort(List, Comparator) sort(List, Comparator)} method), prior to making this call. If it
	 * is not sorted, the results are undefined. If the list contains multiple elements equal to the
	 ...
	 *         only if the key is found.
	 * @throws ClassCastException if the list contains elements that are not <i>mutually comparable</i>
	 *                            using the specified comparator, or the search key is not mutually
	 ...
	 * @throws UnsupportedOperationException if the specified list or its list-iterator does not support
	 ...
	 * randomly selected from the portion of the list that runs from the first element to the current
	 * position, inclusive.
	 * <p>
	 *
	 * This method runs in linear time. If the specified list does not implement the
	 * {@link RandomAccess} interface and is large, this implementation dumps the specified list into an
	 * array before shuffling it, and dumps the shuffled array back into the list. This avoids the
	 * quadratic behavior that would result from shuffling a "sequential access" list in place.
	 *
	...
	/**
	 * Randomly permute the specified list using the specified source of randomness. All permutations
	...
	/**
	 * Returns the minimum element of the given collection, according to the <i>natural ordering</i> of
	 * its elements. All elements in the collection must implement the <tt>Comparable</tt> interface.
	 * Furthermore, all elements in the collection must be <i>mutually comparable</i> (that is,
	 * <tt>e1.compareTo(e2)</tt> must not throw a <tt>ClassCastException</tt> for any elements
	 * <tt>e1</tt> and <tt>e2</tt> in the collection).
	 * <p>
	 *
	 * This method iterates over the entire collection, hence it requires time proportional to the size
	 * of the collection.
	 *
	 * @param coll the collection whose minimum element is to be determined.
	 * @return the minimum element of the given collection, according to the <i>natural ordering</i> of
	 *         its elements.
	...
	/**
	 * Returns the maximum element of the given collection, according to the <i>natural ordering</i> of
	 * its elements. All elements in the collection must implement the <tt>Comparable</tt> interface.
	 * Furthermore, all elements in the collection must be <i>mutually comparable</i> (that is,
	 * <tt>e1.compareTo(e2)</tt> must not throw a <tt>ClassCastException</tt> for any elements
	 * <tt>e1</tt> and <tt>e2</tt> in the collection).
	 ...
	 * @return the maximum element of the given collection, according to the <i>natural ordering</i> of
	 *         its elements.
	 * @throws ClassCastException     if the collection contains elements that are not <i>mutually
	 *                                comparable</i> (for example, strings and integers).
	 * @throws NoSuchElementException if the collection is empty.
	 ...
	 * the element at index <tt>i</tt> will be the element previously at index <tt>(i - distance)</tt>
	 * mod <tt>list.size()</tt>, for all values of <tt>i</tt> between <tt>0</tt> and
	 ...
	 * <tt>Collections.rotate(list, 1)</tt> (or <tt>Collections.rotate(list, -4)</tt>), <tt>list</tt>
	 * will comprise <tt>[s, t, a, n, k]</tt>.
	 *
	 * <p>
	 * Note that this method can usefully be applied to sublists to move one or more elements within a
	 * list while preserving the order of the remaining elements. For example, the following idiom moves
	 * the element at index <tt>j</tt> forward to position <tt>k</tt> (which must be greater than or
	 * equal to <tt>j</tt>):
	 * 
	 * <pre>
	 ...
	 * element at index <tt>1</tt> (<tt>b</tt>) forward two positions, perform the following invocation:
	 * 
	 * <pre>
	 ...
	 * implementation exchanges the first element into the location it should go, and then repeatedly
	 * exchanges the displaced element into the location it should go until a displaced element is
	 * swapped into the first element. If necessary, the process is repeated on the second and
	 * successive elements, until the rotation is complete. If the specified list is large and doesn't
	 * implement the <tt>RandomAccess</tt> interface, this implementation breaks the list into two
	 * sublist views around index <tt>-distance mod size</tt>. Then the {@link #reverse(List)} method is
	 * invoked on each sublist view, and finally it is invoked on the entire list. For a more complete
	 * description of both algorithms, see Section 2.3 of Jon Bentley's <i>Programming Pearls</i>
	 * (Addison-Wesley, 1986).
	 *
	 * @param list     the list to be rotated.
	 * @param distance the distance to rotate the list. There are no constraints on this value; it may
	 *                 be zero, negative, or greater than <tt>list.size()</tt>.
	 * @throws UnsupportedOperationException if the specified list or its list-iterator does not support
	 *                                       the <tt>set</tt> operation.
	 * @since 1.4
	 */
	...
	/**
	 * Replaces all occurrences of one specified value in a list with another. More formally, replaces
	 * with <tt>newVal</tt> each element <tt>e</tt> in <tt>list</tt> such that
	 * <tt>(oldVal==null ? e==null : oldVal.equals(e))</tt>. (This method has no effect on the size of
	 * the list.)
	 *
	 ...
	 * specified source list, or -1 if there is no such occurrence. More formally, returns the lowest
	 * index <tt>i</tt> such that <tt>source.subList(i, i+target.size()).equals(target)</tt>, or -1 if
	 * there is no such index. (Returns -1 if <tt>target.size() > source.size()</tt>.)
	 *
	 * <p>
	 * This implementation uses the "brute force" technique of scanning over the source list, looking
	 * for a match with the target at each location in turn.
	 *
	 ...
	 * specified source list, or -1 if there is no such occurrence. More formally, returns the highest
	 * index <tt>i</tt> such that <tt>source.subList(i, i+target.size()).equals(target)</tt>, or -1 if
	 * there is no such index. (Returns -1 if <tt>target.size() > source.size()</tt>.)
	 *
	 * <p>
	 * This implementation uses the "brute force" technique of iterating over the source list, looking
	 * for a match with the target at each location in turn.
	 *
	 ...
	 * with "read-only" access to internal sets. Query operations on the returned set "read through" to
	 * the specified set, and attempts to modify the returned set, whether direct or via its iterator,
	...
	/**
	 * Returns an unmodifiable view of the specified sorted set. This method allows modules to provide
	 * users with "read-only" access to internal sorted sets. Query operations on the returned sorted
	 * set "read through" to the specified sorted set. Attempts to modify the returned sorted set,
	 * whether direct, via its iterator, or via its <tt>subSet</tt>, <tt>headSet</tt>, or
	 * <tt>tailSet</tt> views, result in an <tt>UnsupportedOperationException</tt>.
	 * <p>
	 *
	 ...
	 * with "read-only" access to internal lists. Query operations on the returned list "read through"
	 * to the specified list, and attempts to modify the returned list, whether direct or via its
	...
	static class UnmodifiableList<E> extends UnmodifiableCollection<E> implements List<E> {
		...
		/**
		 * UnmodifiableRandomAccessList instances are serialized as UnmodifiableList instances to allow them
		 * to be deserialized in pre-1.4 JREs (which do not have UnmodifiableRandomAccessList). This method
		 * inverts the transformation. As a beneficial side-effect, it also grafts the RandomAccess marker
		 * onto UnmodifiableList instances that were serialized in pre-1.4 JREs.
		 *
		 * Note: Unfortunately, UnmodifiableRandomAccessList instances serialized in 1.4.1 and deserialized
		 * in 1.4 will become UnmodifiableList instances, as this method was missing in 1.4.
		 */
	 ...
	 * with "read-only" access to internal maps. Query operations on the returned map "read through" to
	 * the specified map, and attempts to modify the returned map, whether direct or via its collection
	...
	private static class UnmodifiableMap<K, V> implements Map<K, V>, Serializable {
		...
		/**
		 * We need this class in addition to UnmodifiableSet as Map.Entries themselves permit modification
		 * of the backing Map via their setValue operation. This class is subtle: there are many possible
		 * attacks that must be thwarted.
		 *
		 * @serial include
		 */
		static class UnmodifiableEntrySet<K, V> extends UnmodifiableSet<Map.Entry<K, V>> {
			...
			/**
			 * This method is overridden to protect the backing set against an object with a nefarious equals
			 * function that senses that the equality-candidate is Map.Entry and calls its setValue method.
			 */
			public boolean contains(Object o) {
			...
			/**
			 * The next two methods are overridden to protect against an unscrupulous List whose contains(Object
			 * o) method senses when o is a Map.Entry, and calls o.setValue.
			 */
			public boolean containsAll(Collection<?> coll) {
				for (Object e : coll) {
			...
			/**
			 * This "wrapper class" serves two purposes: it prevents the client from modifying the backing Map,
			 * by short-circuiting the setValue method, and it protects the backing Map against an ill-behaved
			 * Map.Entry that attempts to modify another Map Entry when asked to perform an equality check.
			 */
	...
	/**
	 * Returns an unmodifiable view of the specified sorted map. This method allows modules to provide
	 * users with "read-only" access to internal sorted maps. Query operations on the returned sorted
	 * map "read through" to the specified sorted map. Attempts to modify the returned sorted map,
	 * whether direct, via its collection views, or via its <tt>subMap</tt>, <tt>headMap</tt>, or
	 * <tt>tailMap</tt> views, result in an <tt>UnsupportedOperationException</tt>.
	 * <p>
	 *
	 ...
	 *
	 * <p>
	 * The returned collection does <i>not</i> pass the <tt>hashCode</tt> and <tt>equals</tt> operations
	 * through to the backing collection, but relies on <tt>Object</tt>'s equals and hashCode methods.
	 * This is necessary to preserve the contracts of these operations in the case that the backing
	 ...
	 * serial access, it is critical that <strong>all</strong> access to the backing set is accomplished
	...
	static class SynchronizedList<E> extends SynchronizedCollection<E> implements List<E> {
		...
		/**
		 * SynchronizedRandomAccessList instances are serialized as SynchronizedList instances to allow them
		 * to be deserialized in pre-1.4 JREs (which do not have SynchronizedRandomAccessList). This method
		 * inverts the transformation. As a beneficial side-effect, it also grafts the RandomAccess marker
		 * onto SynchronizedList instances that were serialized in pre-1.4 JREs.
		 *
		 * Note: Unfortunately, SynchronizedRandomAccessList instances serialized in 1.4.1 and deserialized
		 * in 1.4 will become SynchronizedList instances, as this method was missing in 1.4.
		 */
		private Object readResolve() {
	 ...
	 * serial access, it is critical that <strong>all</strong> access to the backing map is accomplished
	 * through the returned map.
	 * <p>
	 *
	 * It is imperative that the user manually synchronize on the returned map when iterating over any
	...
	/**
	 * Returns a dynamically typesafe view of the specified collection. Any attempt to insert an element
	 * of the wrong type will result in an immediate {@link ClassCastException}. Assuming a collection
	 * contains no incorrectly typed elements prior to the time a dynamically typesafe view is
	 * generated, and that all subsequent access to the collection takes place through the view, it is
	 * <i>guaranteed</i> that the collection cannot contain an incorrectly typed element.
	 *
	 * <p>
	 * The generics mechanism in the language provides compile-time (static) type checking, but it is
	 * possible to defeat this mechanism with unchecked casts. Usually this is not a problem, as the
	 * compiler issues warnings on all such unchecked operations. There are, however, times when static
	 * type checking alone is not sufficient. For example, suppose a collection is passed to a
	 ...
	 * parameterized collection. Unfortunately, the exception can occur at any time after the erroneous
	 * element is inserted, so it typically provides little or no information as to the real source of
	 * the problem. If the problem is reproducible, one can quickly determine its source by temporarily
	 * modifying the program to wrap the collection with a dynamically typesafe view. For example, this
	 * declaration:
	 * 
	 * <pre>
	 * {
	 ...
	 * is inserted into the collection, clearly identifying the source of the problem. Once the problem
	 * is fixed, the modified declaration may be reverted back to the original.
	 *
	 * <p>
	 * The returned collection does <i>not</i> pass the hashCode and equals operations through to the
	 * backing collection, but relies on {@code Object}'s {@code equals} and {@code hashCode} methods.
	 * This is necessary to preserve the contracts of these operations in the case that the backing
	 * collection is a set or a list.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Since {@code null} is considered to be a value of any reference type, the returned collection
	 * permits insertion of null elements whenever the backing collection does.
	 *
	...
	/**
	 * Returns a dynamically typesafe view of the specified set. Any attempt to insert an element of the
	 * wrong type will result in an immediate {@link ClassCastException}. Assuming a set contains no
	 * incorrectly typed elements prior to the time a dynamically typesafe view is generated, and that
	 * all subsequent access to the set takes place through the view, it is <i>guaranteed</i> that the
	 * set cannot contain an incorrectly typed element.
	 *
	 * <p>
	 * A discussion of the use of dynamically typesafe views may be found in the documentation for the
	 * {@link #checkedCollection checkedCollection} method.
	 *
	 * <p>
	 * The returned set will be serializable if the specified set is serializable.
	 *
	 * <p>
	 * Since {@code null} is considered to be a value of any reference type, the returned set permits
	 * insertion of null elements whenever the backing set does.
	 *
	...
	/**
	 * Returns a dynamically typesafe view of the specified sorted set. Any attempt to insert an element
	 * of the wrong type will result in an immediate {@link ClassCastException}. Assuming a sorted set
	 * contains no incorrectly typed elements prior to the time a dynamically typesafe view is
	 * generated, and that all subsequent access to the sorted set takes place through the view, it is
	 * <i>guaranteed</i> that the sorted set cannot contain an incorrectly typed element.
	 *
	 * <p>
	 * A discussion of the use of dynamically typesafe views may be found in the documentation for the
	 * {@link #checkedCollection checkedCollection} method.
	 ...
	 *
	 * <p>
	 * Since {@code null} is considered to be a value of any reference type, the returned sorted set
	 * permits insertion of null elements whenever the backing sorted set does.
	 *
	 ...
	 * that all subsequent access to the list takes place through the view, it is <i>guaranteed</i> that
	 ...
	 *
	 * <p>
	 * Since {@code null} is considered to be a value of any reference type, the returned list permits
	 ...
	 * immediate {@link ClassCastException}, whether the modification is attempted directly through the
	 * map itself, or through a {@link Map.Entry} instance obtained from the map's {@link Map#entrySet()
	 * entry set} view.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * Since {@code null} is considered to be a value of any reference type, the returned map permits
	 * insertion of null keys or values whenever the backing map does.
	 *
	...
	private static class CheckedMap<K, V> implements Map<K, V>, Serializable {
		...
		/**
		 * We need this class in addition to CheckedSet as Map.Entry permits modification of the backing Map
		 * via the setValue operation. This class is subtle: there are many possible attacks that must be
		 * thwarted.
		 *
		 * @serial exclude
		 */
		static class CheckedEntrySet<K, V> implements Set<Map.Entry<K, V>> {
			...
			public Object[] toArray() {
				...
				/*
				 * Ensure that we don't get an ArrayStoreException even if s.toArray returns an array of something
				 * other than Object
				 */
	...
	/**
	 * Returns a dynamically typesafe view of the specified sorted map. Any attempt to insert a mapping
	 * whose key or value have the wrong type will result in an immediate {@link ClassCastException}.
	 * Similarly, any attempt to modify the value currently associated with a key will result in an
	 * immediate {@link ClassCastException}, whether the modification is attempted directly through the
	 * map itself, or through a {@link Map.Entry} instance obtained from the map's {@link Map#entrySet()
	 * entry set} view.
	 ...
	 *
	 * <p>
	 * Implementations of this method are permitted, but not required, to return the same object from
	...
	/**
	 * Returns the empty set (immutable). This set is serializable. Unlike the like-named field, this
	 ...
	 * object for each call. Using this method is likely to have comparable cost to using the like-named
	...
	/**
	 * Returns an immutable map, mapping only the specified key to the specified value. The returned map
	 * is serializable.
	 *
	 ...
	 * allocated data object is tiny (it contains a single reference to the data object). This method is
	 * useful in combination with the <tt>List.addAll</tt> method to grow lists. The returned list is
	 * serializable.
	 *
	...
	/**
	 * Returns a comparator that imposes the reverse of the <em>natural ordering</em> on a collection of
	 * objects that implement the {@code Comparable} interface. (The natural ordering is the ordering
	 * imposed by the objects' own {@code compareTo} method.) This enables a simple idiom for sorting
	 * (or maintaining) collections (or arrays) of objects that implement the {@code Comparable}
	 * interface in reverse-natural-order. For example, suppose {@code a} is an array of strings. Then:
	 * 
	 * <pre>
	 ...
	 * specified comparator is {@code null}, this method is equivalent to {@link #reverseOrder()} (in
	 * other words, it returns a comparator that imposes the reverse of the <em>natural ordering</em> on
	...
	private static class ReverseComparator2<T> implements Comparator<T>, Serializable {
		...
		/**
		 * The comparator specified in the static factory. This will never be null, as the static factory
		 * returns a ReverseComparator instance if its argument is null.
		 *
		 * @serial
		 */
	...
	/**
	 * Returns an enumeration over the specified collection. This provides interoperability with legacy
	 ...
	 * they are returned by the enumeration. This method provides interoperability between legacy APIs
	 ...
	 * formally, returns the number of elements <tt>e</tt> in the collection such that
	 ...
	 *
	 * <p>
	 * Care must be exercised if this method is used on collections that do not comply with the general
	 * contract for {@code Collection}. Implementations may elect to iterate over either collection and
	 * test for containment in the other collection (or to perform any equivalent computation). If
	 * either collection uses a nonstandard equality test (as does a {@link SortedSet} whose ordering is
	 * not <em>compatible with equals</em>, or the key set of an {@link IdentityHashMap}), both
	 * collections must use the same nonstandard equality test, or the result of this method is
	 * undefined.
	 *
	 * <p>
	 * Care must also be exercised when using collections that have restrictions on the elements that
	 * they may contain. Collection implementations are allowed to throw exceptions for any operation
	 * involving elements they deem ineligible. For absolute safety the specified collections should
	 * contain only elements which are eligible elements for both collections.
	 *
	 * <p>
	 * Note that it is permissible to pass the same collection in both parameters, in which case the
	 * method will return {@code true} if and only if the collection is empty.
	 *
	 ...
	 *                              ineligible for the other collection.
	 *                              (<a href="Collection.html#optional-restrictions">optional</a>)
	 * @since 1.5
	 */
	 ...
	 * specified individually or as an array. The behavior of this convenience method is identical to
	 * that of <tt>c.addAll(Arrays.asList(elements))</tt>, but this method is likely to run
	 ...
	 *                                       <tt>c</tt> does not permit null elements, or if <tt>c</tt>
	 *                                       or <tt>elements</tt> are <tt>null</tt>
	 * @throws IllegalArgumentException      if some property of a value in <tt>elements</tt> prevents
	 *                                       it from being added to <tt>c</tt>
	 ...
	 * concurrency, and performance characteristics as the backing map. In essence, this factory method
	 * provides a {@link Set} implementation corresponding to any {@link Map} implementation. There is
	 * no need to use this method on a {@link Map} implementation that already has a corresponding
	 * {@link Set} implementation (such as {@link HashMap} or {@link TreeMap}).
	 *
	 * <p>
	 ...
	 * <tt>addAll</tt> method is implemented as a sequence of <tt>put</tt> invocations on the backing
	 * map.
	 *
	 * <p>
	 * The specified map must be empty at the time this method is invoked, and should not be accessed
	 * directly after this method returns. These conditions are ensured if the map is created empty,
	 * passed directly to this method, and no reference to the map is retained, as illustrated in the
	 ...
	 * This view can be useful when you would like to use a method requiring a <tt>Queue</tt> but you
	 * need Lifo ordering.
	 *
	 * <p>
	 * Each method invocation on the queue returned by this method results in exactly one method
	 * invocation on the backing deque, with one exception. The {@link Queue#addAll addAll} method is
