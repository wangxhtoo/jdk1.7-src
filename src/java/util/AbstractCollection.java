...
public abstract class AbstractCollection<E> implements Collection<E> {
	 ...
	 * of the array, starting with index {@code 0}. The length of the returned array
	 * is equal to the number of elements returned by the iterator, even if the size
	 * of this collection changes during iteration, as might happen if the
	 * collection permits concurrent modification during iteration. The {@code size}
	 ...
	 * This implementation returns an array containing all the elements returned by
	 * this collection's iterator in the same order, stored in consecutive elements
	 * of the array, starting with index {@code 0}. If the number of elements
	 * returned by the iterator is too large to fit into the specified array, then
	 * the elements are returned in a newly allocated array with length equal to the
	 * number of elements returned by the iterator, even if the size of this
	 * collection changes during iteration, as might happen if the collection
	 * permits concurrent modification during iteration. The {@code size} method is
	 * called only as an optimization hint; the correct result is returned even if
	 * the iterator returns a different number of elements.
	 *
	 * <p>
	 * This method is equivalent to:
	 *
	...
	/**
	 * Reallocates the array being used within toArray when the iterator returned
	 * more elements than expected, and finishes filling it from the iterator.
	 *
	 * @param r  the array, replete with previously stored elements
	 * @param it the in-progress iterator over this collection
	 * @return array containing the elements in the given array, plus any further
	 *         elements returned by the iterator, trimmed to size
	 */
	 ...
	 * Note that this implementation will throw an
	 * <tt>UnsupportedOperationException</tt> if the iterator returned by this
	 * collection's <tt>iterator</tt> method does not implement the <tt>remove</tt>
