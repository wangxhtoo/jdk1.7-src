...
public class Executors {
	 ...
	 * shared unbounded queue. At any point, at most <tt>nThreads</tt> threads will
	 ...
	 * threads when needed. At any point, at most <tt>nThreads</tt> threads will be
	 ...
	 * Unlike the otherwise equivalent <tt>newFixedThreadPool(1)</tt> the returned
	 ...
	 * asynchronous tasks. Calls to <tt>execute</tt> will reuse previously
	 ...
	 * <tt>newScheduledThreadPool(1)</tt> the returned executor is guaranteed not to
	 ...
	 * guaranteed to execute sequentially, and no more than one task will be active
	 * at any given time. Unlike the otherwise equivalent
	 * <tt>newScheduledThreadPool(1, threadFactory)</tt> the returned executor is
	 * guaranteed not to be reconfigurable to use additional threads.
	 * 
	 * @param threadFactory the factory to use when creating new threads
	 ...
	 * <tt>defaultThreadFactory</tt> method. Each new thread is created as a
	 ...
	 * thread group. New threads have names accessible via {@link Thread#getName} of
	 ...
	 * the thread invoking this <tt>privilegedThreadFactory</tt> method. A new
	 * <tt>privilegedThreadFactory</tt> can be created within an
	 * {@link AccessController#doPrivileged} action setting the current thread's
	 * access control context to create threads with the selected permission
	 ...
	 * {@link ThreadPoolExecutor#beforeExecute}. Also, if it is necessary to
	 ...
	 * that thread waits for and services requests to create others that will
	 * inherit its values.
	 *
	 * @return a thread factory
	 ...
	 * a <tt>Callable</tt> to an otherwise resultless action.
	 * 
	 ...
	 * <tt>callable</tt> under the current access control context. This method
	 ...
	 * associated {@link AccessControlException}.
	 * 
	...
	/**
	 * Returns a {@link Callable} object that will, when called, execute the given
	 * <tt>callable</tt> under the current access control context, with the current
	 * context class loader as the context class loader. This method should normally
	 * be invoked within an {@link AccessController#doPrivileged} action to create
	 * callables that will, if possible, execute under the selected permission
