 ...
 * policy uses the environment property "{@link Context#INITIAL_CONTEXT_FACTORY
 * java.naming.factory.initial}", which contains the class name of the initial
 * context factory. An exception to this policy is made when resolving URL
...
public class InitialContext implements Context {
	 ...
	 * null and is updated by the constructor that accepts an environment or by the
	 * <tt>init()</tt> method.
	 * 
	...
	/**
	 * Field holding the result of calling NamingManager.getInitialContext(). It is
	 * set by getDefaultInitCtx() the first time getDefaultInitCtx() is called.
	 * Subsequent invocations of getDefaultInitCtx() return the value of
	 * defaultInitCtx.
	 * 
	...
	/**
	 * Constructs an initial context with the option of not initializing it. This
	 * may be used by a constructor in a subclass when the value of the environment
	 * parameter is not yet known at the time the <tt>InitialContext</tt>
	 * constructor is called. The subclass's constructor will call this constructor,
	 * compute the value of the environment, and then call <tt>init()</tt> before
	 * returning.
	 *
	 * @param lazy true means do not initialize the initial context; false is
	 ...
	 *
	 * <p>
	 * This constructor will not modify <tt>environment</tt> or save a reference to
	 * it, but may save a clone. Caller should not modify mutable keys and values in
	 * <tt>environment</tt> after it has been passed to the constructor.
	 *
	 * @param environment environment used to create the initial context. Null
	 ...
	 *
	 * <p>
	 * This method will modify <tt>environment</tt> and save a reference to it. The
	 ...
	 * represents the same naming context as this context, but its environment may
	 * be modified independently and it may be accessed concurrently).
	 *
	...
	/**
	 * A static method to retrieve the named object. See {@link #doLookup(Name)} for
	 * details.
	 * 
	 * @param name the name of the object to look up
	 * @return the object bound to <tt>name</tt>
	 ...
	 * defaultInitCtx. Set <code>gotDefault</code> so that we know we've tried this
	 * before.
	 * 
	 ...
	 * <code>name</code> name is a URL string, then attempt to find a URL context
	 * for it. If none is found, or if <code>name</code> is not a URL string, then
	 ...
	 * @return A URL context for <code>name</code> or the cached initial context.
	...
	/**
	 * Retrieves a context for resolving <code>name</code>. If the first component
	 * of <code>name</code> name is a URL string, then attempt to find a URL context
	 * for it. If none is found, or if the first component of <code>name</code> is
	 * not a URL string, then return <code>getDefaultInitCtx()</code>.
	 * <p>
	 * When creating a subclass of InitialContext, use this method as follows.
	 * Define a new method that uses this method to get an initial context of the
	 * desired subclass.
	 * <p>
	 * <blockquote>
	 * 
...
// Most Javadoc is deferred to the Context interface.
	 ...
	 * itself, the value of the <tt>prefix</tt> parameter must be an empty name
	 * (<tt>""</tt>).
	 */
